...
KARA_LISTE = [
    "topluluk kuralları", "çerez", "veri politikası",
    "kullanım şartları", "tüm hakları saklıdır", "copyright",
    "yorum yazarak", "ajans kanallarından", "editöryel müdahale",
    "oturum açın", "buraya tıklayın", "yorumunuz için teşekkür",
    "abone ol", "bülteni", "reklam",
]
...
class CagdasKocaeliScraper(BaseScraper):
    ...
    def __init__(self):
        super().__init__(
            site_adi="Çağdaş Kocaeli",
            ana_url="https://www.cagdaskocaeli.com.tr/"
        )

    def paragraf_temiz_mi(self, metin):
        metin_kucuk = metin.lower()
        for ifade in KARA_LISTE:
    ...
    def haber_detay_cek(self, url):
        ...
        icerik_alani = (
            soup.find("div", class_="news-detail-content") or
            soup.find("div", class_="haber-detay") or
            soup.find("div", class_="detay-icerik") or
            soup.find("article")
        )
        kaynak = icerik_alani if icerik_alani else soup
        paragraflar = [
            p.text.strip() for p in kaynak.find_all("p")
            if len(p.text.strip()) > 40 and self.paragraf_temiz_mi(p.text)
        ]
    ...
    def scrape(self):
        ...
        for a in soup.find_all("a", href=True):
            link = a["href"]
            if "/haber/" in link and len(link) > 20:
                linkler.add(urljoin(self.ana_url, link))
